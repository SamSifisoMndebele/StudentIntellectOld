...
class PdfFileFragment : Fragment() {

    companion object {
        private const val ARG_PDF_FILE = "arg_pdf_file"
        private const val ARG_PARENT_FOLDER = "arg_parent_folder"
        @JvmStatic
        fun newInstance(pdfFile: PdfFile, folder : Folder): PdfFileFragment {
            return PdfFileFragment().apply {
                arguments = Bundle().apply {
                    putParcelable(ARG_PDF_FILE, pdfFile)
                    putParcelable(ARG_PARENT_FOLDER, folder)
                }
            }
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        storageRef = Firebase.storage.getReferenceFromUrl(pdfFile.materialUrl)
        prefs = activity?.getSharedPreferences("last_page_number",Context.MODE_PRIVATE)
        pageKey = "${parentFolder.id}_${pdfFile.id}"
    ...
    private fun checkIfPdfFileExistsAndOpenOrDownload(page: Int = 0, i : Int = 0) {
        ...
        file = if (i == 0) {
            File(Utils.filesDir(filePath), "${pdfFile.id}.pdf")
        } else {
            File(Utils.filesDir(filePath), "${pdfFile.id}(${i}).pdf")
        }
    ...
    private fun FileDownloadTask.addTaskListeners(continueTask : Boolean = false) {
        this
            .addOnSuccessListener {
                ...
                val filesTableName = parentFolder.path.replace(" ", "")
                    .replace("Modules", "Files")
                    .replace("/", "_")+"_Table"
                FilesFirestoreDatabase(requireContext(), parentFolder.path,  filesTableName).updateDownloads(pdfFile)
            }
